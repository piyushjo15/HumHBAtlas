### getting chromvar deviation for progenitors
...
proj <- loadArchRProject(path = "Subset_Progenitors/")
gr_obj =  rtracklayer::import("PeakCom/AllPeaks_robust.fil.strtmin1.bed") 

proj <- addPeakSet(proj, peakSet = gr_obj, force = TRUE)

#Adding Peak matrix
proj <- addPeakMatrix(proj,
                      ceiling = 8, ##changed it to 8
                      binarize = F,
                      force = T)
...
pfms_ret <- homerToPFMatrixList("~/MBsnANA/HBana/ATACana/Outs/extrafiles/allpat_lstmv2_57MP_73_220725.motif", n = 100L)
pwms <- toPWM(pfms_ret)
...
deephb <- read_homer("~/MBsnANA/HBana/ATACana/Outs/extrafiles/allpat_lstmv2_57MP_73_220725.motif")
...
save(mat_motif, file = "extrafiles/Imputed_motif_mat_allpat_pro.RData")
...
save(mat_motif, file = "extrafiles/Imputed_motif_matZ_allpat_pro.RData")
q()
