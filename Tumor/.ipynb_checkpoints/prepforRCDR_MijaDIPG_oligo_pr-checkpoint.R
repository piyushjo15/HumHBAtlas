...
k3 <- grep("Microglia",plot.data$ANN2)
plot.data <- plot.data[c(k1,k2,k3),]
plot.data <- plot.data[c(k2),]
...
if(length(cells)>3000){
  cells <- sample(cells,3000)
}
...
np.save(f"genes_pc_pr.npy", genes_sel)
exit
