## this sript generate GN for each sample , I have to grph as edge list for pecanpy
import os
import sys
...
mat_tum = load_npz(f"genemat_"+SAM+"_pc_pr.npz")
genes_tum = np.load(f"genes_pc_pr.npy", allow_pickle=True)
print(mat_tum.shape)
print(genes_tum[0:5])
print(len(genes_tum))
...
#  Save
with open(f"gene_edge_list_{SAM}_pr.pkl", "wb") as f:
    pickle.dump(df_tum, f)

## 1.3 Create graph from adjacency list
## reading edge list
#df_tum= pd.read_pickle(f"gene_edge_list_{SAM}_pr.pkl")
...
nx.write_gexf(G_tum, f"gene_network_{SAM}_pr.gexf")
nx.write_edgelist(G_tum, f"gene_network_{SAM}_pr.edg", delimiter='\t', data=['weight'])
with open(f"gene_network_{SAM}_pr.pkl", "wb") as f:
    pickle.dump(G_tum, f)


# Save filtered communtiies
df_partition = pd.DataFrame(list(part_tum.items()), columns=["gene", "module"])
df_partition.to_csv(f"partition_{SAM}_pr.csv", index=False)
# 
np.save(f"genes_fil_{SAM}_pr.npy", np.array(df_partition.gene))
