...
##this script calculates context drift scores for genes in tumor
## by caomparing to grapf in the normal
## 3 separate graphs are created for normal, each comparison is
## run separately and an average scaore is calculcated later
partx='a' ## 'b' or 'c'
## load the cosine drift scores
with open(f"PA_cosines_{partx}.pkl", "rb") as f:
  df=pickle.load(f)
#with open(f"DMG_cosines_{partx}.pkl", "rb") as f:
#  df=pickle.load(f)
...
with open(f"Normal/gene_network_nor_{partx}.pkl", "rb") as f:
  G_nor=pickle.load(f)
#with open(f"Normal_PFA/gene_network_nor_{partx}.pkl", "rb") as f:
#  G_nor=pickle.load(f)
with open(f"{SAM}/gene_network_{SAM}.pkl", "rb") as f:
    G_tum=pickle.load(f)
...
df_scores.to_csv(f"{SAM}/gene_context_drift_scores_nor_{SAM}_{partx}.csv", index=False)
